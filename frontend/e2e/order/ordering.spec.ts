...
test.describe("Order — Customer Ordering App", () => {
  const cartHeading = (count: number) =>
    new RegExp(
      fixture.order.cartHeadingTemplate.replace("{n}", String(count)),
      "i",
    );
  ...
  test("homepage shows branch selection", async ({ page, request }) => {
    ...
    if (branchesData[0]) {
      await expect(
        page.getByRole("button", { name: branchesData[0].name }),
      ).toBeVisible();
    }
    if (branchesData[1]) {
      await expect(
        page.getByRole("button", { name: branchesData[1].name }),
      ).toBeVisible();
    }
    if (branchesData[2]) {
      await expect(
        page.getByRole("button", { name: branchesData[2].name }),
      ).toBeVisible();
    }
  });
  ...
  test("selecting a branch navigates to menu", async ({ page, request }) => {
    ...
    if (branches[0]) {
      await page
        .getByRole("button", { name: new RegExp(branches[0].name) })
        .click();
    }
  ...
  test("menu page shows products", async ({ page, request }) => {
    ...
    await page.goto("/");
    await page
      .getByRole("button", { name: new RegExp(firstBranchName) })
      .click();
    await expect(page).toHaveURL(/\/menu/);
  ...
  test("authenticated customer can add, review, and place an order", async ({
    ...
    await page
      .getByRole("button", { name: fixture.order.increaseQtyButton })
      .click();
    await page
      .getByRole("button", { name: fixture.order.addToCartButton })
      .click();
    await expect(
      page.getByText(fixture.order.addToCartConfirmation),
    ).toBeVisible();

    await page
      .getByRole("button", { name: fixture.order.viewCartButton })
      .click();
    await expect(page).toHaveURL(/\/cart/);
    await expect(
      page.getByRole("heading", { name: cartHeading(2) }),
    ).toBeVisible();
    await page
      .getByRole("button", { name: fixture.order.increaseQtyButton })
      .click();
    await expect(
      page.getByRole("heading", { name: cartHeading(3) }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", {
        name: new RegExp(fixture.order.payAtCounterButton, "i"),
      }),
    ).toBeVisible();
    await page
      .getByRole("button", { name: fixture.order.deliveryButton })
      .click();
    await expect(
      page.getByText(fixture.order.deliveryAddressLabel),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", {
        name: new RegExp(fixture.order.orderPlacedHeading, "i"),
      }),
    ).toBeVisible({ timeout: 10000 });
    await expect(
      page.getByRole("link", {
        name: new RegExp(fixture.order.trackOrderLink, "i"),
      }),
    ).toBeVisible();
  });
});
