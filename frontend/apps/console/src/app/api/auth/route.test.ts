...
describe("Admin Route Auth Gating Logic", () => {
  ...
  describe("POST handler", () => {
    ...
    it("POST non-staff JWT (roles ['customer']) -> status 403 AND no console_token cookie set", async () => {
    ...
    it("POST staff JWT -> status 200 AND httpOnly console_token cookie set", async () => {
      ...
      expect(mockSet).toHaveBeenCalledWith("console_token", staffToken, expect.objectContaining({ httpOnly: true }));
    });
    ...
    it("action:'logout' -> console_token cookie deleted", async () => {
      const req = new NextRequest("http://localhost/api/auth", {
        method: "POST",
      ...
      const res = await POST(req);
      expect(res.status).toBe(200);
      expect(mockDelete).toHaveBeenCalledWith("console_token");
    });
  });
});
