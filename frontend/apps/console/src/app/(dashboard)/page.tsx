...
function ProductManagerDashboard() {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div ref={panelRef} className="min-h-0 flex-1 overflow-hidden rounded-lg border border-[var(--console-line)] bg-white p-4">
        ...
        <table className="w-full text-left text-[12px]">
          ...
          <tbody>
            {topProducts?.items?.slice(0, listSize).map((s: ProductStat, i: number) => (
              <tr key={s.id} className="h-12 border-b border-[var(--console-line)] last:border-0 hover:bg-[var(--console-panel)] transition-colors">
                <td className="font-mono text-[11px] font-bold" style={{ color: i < 3 ? "var(--cinnamon)" : "var(--console-muted)" }}>
                  {String(i + 1).padStart(2, "0")}
                </td>
                <td>
                  <Link href={`/products/${s.slug}`} className="font-semibold text-espresso hover:underline">{s.name}</Link>
                </td>
                <td className="text-right font-mono text-[var(--console-muted)]">{formatCompactVnd(s.price)}</td>
...
function BranchManagerDashboard() {
  ...
  if (!branchId) {
    return (
      <div className="flex h-full flex-col">
        <p className="font-editorial italic text-[var(--console-muted)]">No branch assigned to your account.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="mb-3.5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {KPIS.map((k) => (
          <div key={k.label} className="rounded-lg border border-[var(--console-line)] bg-white p-4">
            ...
            <div className="mt-1 font-editorial text-[12px] italic text-[var(--console-muted)]">{k.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-hidden rounded-lg border border-[var(--console-line)] bg-white p-4">
        ...
        <table className="w-full text-left text-[12px]">
          ...
          <tbody>
            {data?.top_products?.slice(0, 5).map((p, i) => (
              <tr key={p.product_id} className="h-12 border-b border-[var(--console-line)] last:border-0 hover:bg-[var(--console-panel)] transition-colors">
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-3">
        {/* Top sellers */}
        <div ref={panelRef} className="overflow-hidden rounded-lg border border-[var(--console-line)] bg-white p-4">
          <div className="mb-3 flex items-baseline justify-between">
          ...
          {topProducts?.items?.slice(0, listSize).map((s, i) => (
            <div key={s.id} className={`flex h-12 items-center gap-2.5 ${i === 0 ? "" : "border-t border-[var(--console-line)]"}`}>
              <span className="w-[22px] font-mono text-[11px] font-bold" style={{ color: i < 3 ? "var(--cinnamon)" : "var(--console-muted)" }}>
                {String(i + 1).padStart(2, "0")}
              </span>
              <div className="min-w-0 flex-1 text-[12.5px] font-semibold leading-tight text-espresso truncate">{s.name}</div>
              <div className="flex-shrink-0 text-right font-mono text-[9.5px] tracking-[0.06em] text-[var(--console-muted)]">
        ...
        {/* Recent orders */}
        <div className="overflow-hidden rounded-lg border border-[var(--console-line)] bg-white p-4">
          <div className="mb-3 flex items-baseline justify-between">
          ...
          {recentOrders?.items?.length === 0 && (
            <p className="py-4 text-center font-editorial text-[13px] italic text-[var(--console-muted)]">No orders yet.</p>
          )}
          {recentOrders?.items?.slice(0, listSize).map((o, i) => (
            <div key={o.id} className={`flex h-12 items-center gap-2.5 text-[12px] ${i === 0 ? "" : "border-t border-[var(--console-line)]"}`}>
              <span className="font-mono font-semibold text-espresso">{o.id.slice(0, 8)}</span>
              <span className="flex-1 truncate font-editorial italic text-[var(--console-muted)]">{o.status}</span>
        ...
        <div className="overflow-hidden rounded-lg border border-[var(--console-line)] bg-white p-4">
          ...
          {branchStats?.items?.slice(0, listSize).map((b, i) => (
            <div key={b.branch_id} className={`flex h-12 items-center gap-2.5 ${i === 0 ? "" : "border-t border-[var(--console-line)]"}`}>
              <span className="w-[22px] font-mono text-[11px] font-bold" style={{ color: i < 3 ? "var(--cinnamon)" : "var(--console-muted)" }}>
                {String(i + 1).padStart(2, "0")}
              </span>
              <div className="min-w-0 flex-1">
                <div className="text-[12.5px] font-semibold leading-tight text-espresso">{b.branch_name}</div>
                <div className="mt-0.5 flex gap-2 font-mono text-[9.5px] tracking-[0.06em] text-[var(--console-muted)]">
                  <span>{b.order_count} orders</span>·<span>{b.staff_count} staff</span>
                </div>
              </div>
              <div className="w-[60px]">
                <div className="h-1 rounded-sm bg-[var(--console-panel)]">
                  <div className="h-full rounded-sm bg-cinnamon" style={{ width: `${(b.revenue / maxBranchRev) * 100}%` }} />
                </div>
                <div className="mt-0.5 text-right font-mono text-[9px] text-[var(--console-muted)]">{formatCompactVnd(b.revenue)}</div>
              </div>
            </div>
          ))}
        </div>
