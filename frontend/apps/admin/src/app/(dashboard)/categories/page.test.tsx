...
import { createCategory, updateCategory, deleteCategory } from '@repo/api-client';
...
vi.mock('@tanstack/react-query', () => ({
  useQuery: vi.fn(() => ({
    data: [{ id: 'cat-1', name: 'Bread', slug: 'bread', sort_order: 1, is_active: true }],
    isLoading: false,
  })),
...
describe('CategoriesPage CRUD flow', () => {
  ...
  it('submits createCategory successfully', async () => {
    ...
    await waitFor(() => {
      expect(createCategory).toHaveBeenCalledWith({ name: 'Pastries', sort_order: 2 });
  ...
  it('handles createCategory failure gracefully', async () => {
    vi.mocked(createCategory).mockRejectedValue(new Error('Category creation failed'));
    const { container } = render(<CategoriesPage />);

    fireEvent.click(screen.getByRole('button', { name: /add category/i }));
    fireEvent.change(container.querySelector('input[name="name"]')!, { target: { value: 'Failing Category' } });

    fireEvent.submit(container.querySelector('form')!);

    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith('Category creation failed', 'error');
    });
  });
  ...
  it('submits updateCategory successfully when editing', async () => {
    ...
    await waitFor(() => {
      expect(updateCategory).toHaveBeenCalledWith('cat-1', { name: 'Sourdough Bread', sort_order: 1 });
      expect(mockToast).toHaveBeenCalledWith('Category updated');
    });
  });

  it('handles updateCategory failure', async () => {
    vi.mocked(updateCategory).mockRejectedValue(new Error('Update network error'));
    const { container } = render(<CategoriesPage />);
