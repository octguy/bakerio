...
import {
  getKitchenTickets,
  getKitchenCounts,
} from "@repo/api-client/mock/kitchen";
import type {
  KitchenTicket,
  KitchenStation,
} from "@repo/api-client/mock/kitchen";
...
> = {
  fresh: {
    stripe: "var(--sage)",
    tagBg: "rgba(107,143,94,0.16)",
    tagC: "var(--sage)",
    label: "NEW",
  },
  normal: {
    stripe: "var(--cinnamon)",
    tagBg: "rgba(212,148,58,0.16)",
    tagC: "var(--cinnamon)",
    label: "COOK",
  },
  warning: {
    stripe: "var(--golden)",
    tagBg: "rgba(212,148,58,0.2)",
    tagC: "var(--cinnamon)",
    label: "VIP",
  },
  late: {
    stripe: "var(--sienna)",
    tagBg: "rgba(196,91,74,0.18)",
    tagC: "var(--sienna)",
    label: "LATE",
  },
};
...
export default function KitchenPage() {
  ...
  const fetchKitchenData = async (showLoading = false) => {
    ...
    setError("");
    try {
      const ticketsData = await getKitchenTickets(
      ...
      setError(
        "Could not load kitchen tickets. Retry when the API is reachable.",
      );
  ...
  return (
    <div className="-mx-7 -mt-6 flex h-[calc(100vh-60px)] flex-col bg-cream">
      ...
      <header className="flex items-center gap-6 bg-espresso px-8 py-5 text-cream">
        ...
        <span className="rounded-full bg-honey/18 px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-honey">
          Demo board
        </span>
          ...
          <button
            ...
            className={`inline-flex items-center gap-2.5 rounded-full px-4 py-2 text-[14px] tracking-wide ${
              station === tab.s
                ...
                : "border border-crust bg-white font-semibold text-espresso"
            }`}
          >
            {tab.l}
            <span
      ...
      {error && (
        <div
          role="alert"
          className="mx-8 mt-4 rounded-lg border border-sienna/30 bg-sienna/10 px-4 py-3"
        >
            ...
            return (
              <div
                ...
                style={{
                  border:
                    o.sev === "late"
                      ? "2px solid var(--sienna)"
                      : "1px solid var(--crust)",
                  boxShadow:
                    o.sev === "late"
                      ? "0 0 0 4px rgba(196,91,74,0.1)"
                      : "0 6px 14px -8px rgba(44,24,16,0.12)",
                }}
              >
                <div
                ...
                <div className="flex flex-1 flex-col p-5">
                  ...
                  <div className="mb-2.5 flex items-center gap-2.5">
                    <span className="font-display text-[26px] leading-none tracking-tight text-espresso">
                      #{o.id}
                    </span>
                    <span
                      ...
                      style={{
                        background:
                          o.mode === "DEL"
                            ? "rgba(212,148,58,0.18)"
                            : "rgba(166,139,107,0.16)",
                        color:
                          o.mode === "DEL"
                            ? "var(--cinnamon)"
                            : "var(--caramel)",
                    ...
                    <span
                      className="ml-auto font-display text-[30px] leading-none tracking-tight"
                      style={{ color: sev.stripe }}
                    >
                      {o.mins}
                      <span className="ml-1 text-[16px] text-[var(--admin-muted)]">
                        m
                      </span>
                    </span>
                  </div>
                  ...
                  <div className="mb-3.5 flex flex-1 flex-col gap-1.5">
                    {o.items.map((it, i) => (
                      <div
                        key={i}
                        className={`flex items-start gap-3 py-2 ${i === 0 ? "" : "border-t border-crust"}`}
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="text-[18px] font-semibold leading-[1.15] text-espresso">
                            {it.n}
                          </div>
                          {it.mod && (
                            <div className="mt-0.5 font-editorial text-[13px] italic text-cinnamon">
                              ↳ {it.mod}
                            </div>
                          )}
                        </div>
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      type="button"
                      disabled
                      className="flex-1 cursor-not-allowed rounded-lg bg-espresso px-3 py-3.5 font-mono text-[14px] font-bold uppercase tracking-[0.06em] text-white opacity-45"
                    >
                      ✓ Mark ready
                    </button>
                    <button
                      type="button"
                      disabled
                      aria-label="Return ticket to queue unavailable in demo"
                      className="cursor-not-allowed rounded-lg border border-crust bg-white px-4 py-3.5 font-mono text-[12px] font-bold tracking-[0.12em] text-espresso opacity-45"
                    >
                      ↺
                    </button>
                  </div>
