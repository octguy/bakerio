...
const heatColor = (v: number) => {
  const stops = ["#F5EBD9", "#F1DDB6", "#EBC892", "#E5B26B", "#D4943A", "#B5722A", "#8A4D14"];
...
function Spark({ data, color }: { data: number[]; color: string }) {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
      <polyline points={`0,${h} ${pts} ${w},${h}`} fill={color} opacity="0.16" />
      <polyline
        className="bkr-draw"
        points={pts}
        fill="none"
...
export default function DashboardPage() {
  ...
  const activeCount = products?.filter((p) => p.is_active).length ?? stats.activeProducts;

  const KPIS = [
    { label: "Doanh thu · today", value: "14.6M₫", delta: "+12.4%", sub: "vs 7d avg",
      spark: [12, 14, 11, 13, 16, 14, 15, 17, 14, 15], color: "var(--cinnamon)" },
    { label: "Đơn hàng · today", value: stats.totalOrders.toString(), delta: "+8.1%", sub: "418 forecast",
      spark: [22, 28, 24, 30, 35, 32, 38, 40, 36, 42], color: "var(--sage)" },
    { label: "Trung bình giỏ", value: "128K₫", delta: "+3.6%", sub: "WoW",
      spark: [120, 125, 118, 130, 124, 128, 132, 126, 128, 128], color: "var(--golden)" },
    { label: "Cảnh báo kho", value: stats.lowStockItems.toString(), delta: "+2", sub: "since 06:00",
      spark: [3, 3, 4, 4, 5, 5, 5, 6, 7, 7], color: "var(--sienna)", warn: true },
  ];
  ...
  return (
    <div>
        ...
        <div>
          <div className="mb-1.5 flex items-center gap-3">
            ...
            <span className="font-mono text-[10.5px] uppercase tracking-[0.22em] text-cinnamon">
              Saturday · {new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short" })} · ICT
            </span>
          </div>
          <h1
            className="font-display tracking-tight"
            style={{ fontSize: "clamp(28px,4vw,38px)", lineHeight: 1, letterSpacing: "-0.02em" }}
          >
            Good morning, baker. <span className="font-editorial text-cinnamon">{activeCount} loaves so far.</span>
          </h1>
        </div>
      ...
      <div className="mb-3.5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {KPIS.map((k) => (
          <div key={k.label} className="relative overflow-hidden rounded-lg border border-[var(--admin-line)] bg-white p-4">
      ...
      <div className="mb-3.5 grid grid-cols-1 gap-3 lg:grid-cols-[1.4fr_1fr]">
        ...
        <div className="rounded-lg border border-[var(--admin-line)] bg-white p-5">
          <div className="mb-3 flex items-baseline justify-between">
            <div>
              <div className="font-display text-[18px] tracking-tight">Order density · by hour</div>
            ...
            <div className="flex items-center gap-1.5 font-mono text-[9.5px] tracking-[0.14em] text-[var(--admin-muted)]">
              ...
              {[0.1, 0.3, 0.5, 0.7, 0.9].map((v) => (
                <span key={v} className="block h-3.5 w-3.5 rounded-sm" style={{ background: heatColor(v) }} />
              ))}
              HIGH
            </div>
          </div>
            ...
            <div key={di} className="mb-1 flex items-center gap-1">
              <div
                className={`w-[34px] pr-1.5 text-right font-mono text-[10.5px] tracking-[0.08em] ${
                  di === 5 ? "font-bold text-cinnamon" : "text-[var(--admin-muted)]"
                }`}
              >
                {DAYS[di]}
              </div>
          ...
          <div className="mt-2.5 pl-[36px] font-mono text-[10px] tracking-[0.1em] text-[var(--admin-muted)]">
            Peak: <span className="font-bold text-cinnamon">{heatmapData.peak.day} {String(heatmapData.peak.hour).padStart(2, "0")}:00</span> · {heatmapData.peak.ordersPerHour} orders / hour
          </div>
        </div>
        ...
        <div className="rounded-lg border border-[var(--admin-line)] bg-white p-5">
          <div className="mb-3 flex items-baseline justify-between">
            <div>
              <div className="font-display text-[18px] tracking-tight">Daily revenue</div>
              <div className="font-editorial text-[12px] italic text-[var(--admin-muted)]">
                week · million ₫
              </div>
            </div>
              ...
              {["1W", "2W", "1M", "YTD"].map((tab, i) => (
                <span
                  ...
                  className={`rounded px-2 py-1 font-mono text-[10.5px] tracking-[0.1em] ${
                    i === 0 ? "bg-espresso font-bold text-white" : "text-[var(--admin-muted)]"
                  }`}
                >
                  {tab}
                </span>
          ...
          <svg viewBox="0 0 460 200" className="h-[200px] w-full">
            {[0, 1, 2, 3, 4].map((g) => (
              <line key={g} x1="30" x2="450" y1={20 + g * 36} y2={20 + g * 36} stroke="var(--admin-line)" strokeDasharray="2 4" />
            ))}
            {[30, 22.5, 15, 7.5, 0].map((y, i) => (
              <text key={i} x="22" y={24 + i * 36} className="font-mono" fontSize="9" fill="var(--admin-muted)" textAnchor="end">
                {y}M
              </text>
            ))}
            {dailyRev.map((d, i) => {
              ...
              return (
                <g key={i} className="bkr-grow-up" style={{ animationDelay: `${i * 80}ms`, transformBox: "fill-box", transformOrigin: "50% 100%" }}>
                  <rect x={x} y={top} width="36" height={h} rx="2" fill={today ? "var(--cinnamon)" : "var(--golden)"} opacity={today ? 1 : 0.65} />
                  <text x={x + 18} y={182} className="font-mono" fontSize="9" fill="var(--admin-muted)" textAnchor="middle">
                    {d.date}
                  </text>
                </g>
              );
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-[var(--admin-line)] bg-white p-4">
          <div className="mb-3 flex items-baseline justify-between">
            <div className="font-display text-[17px] tracking-tight">Top sellers · today</div>
            <span className="font-mono text-[10px] font-bold tracking-[0.16em] text-cinnamon">VIEW ALL ↗</span>
          </div>
          {topSellersData.map((s, i) => (
            ...
            >
              <span
                ...
                style={{ color: i < 3 ? "var(--cinnamon)" : "var(--admin-muted)" }}
              >
                {s.rank}
              </span>
              <div className="min-w-0 flex-1">
                <div className="text-[12.5px] font-semibold leading-tight text-espresso">{s.name}</div>
                <div className="mt-0.5 flex gap-2 font-mono text-[9.5px] tracking-[0.06em] text-[var(--admin-muted)]">
                  <span>{s.sold} sold</span>·<span>{formatCurrency(s.rev)}</span>
                </div>
              </div>
              <div className="w-[60px]">
                <div className="h-1 rounded-sm bg-[var(--admin-panel)]">
                  <div className="h-full rounded-sm bg-cinnamon" style={{ width: `${s.share}%` }} />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="rounded-lg border border-[var(--admin-line)] bg-white p-4 lg:col-span-1">
          <div className="mb-3 flex items-baseline justify-between">
            ...
            <span className="font-mono text-[10px] font-bold tracking-[0.16em] text-cinnamon">ALL →</span>
          </div>
          {recent.map((o, i) => (
            <div
              key={o.id}
            ...
            >
              <span className="font-mono font-semibold text-espresso">{o.id}</span>
              <span className="flex-1 font-editorial italic text-[var(--admin-muted)]">{o.customer}</span>
              <span className="font-mono text-espresso">{formatCurrency(o.total).replace("₫", "")}₫</span>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-lg border border-[var(--admin-line)] bg-white p-4">
          <div className="mb-3 flex items-baseline justify-between">
            <div className="font-display text-[17px] tracking-tight">Live alerts</div>
          ...
          {alertsData.map((a, i) => (
            <div key={i} className={`flex gap-2.5 py-2 ${i === 0 ? "" : "border-t border-[var(--admin-line)]"}`}>
              <div
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-0.5 flex items-center gap-2">
                  <span className="font-mono text-[9px] font-bold tracking-[0.2em]" style={{ color: SEV[a.sev] }}>
                    {a.tag}
                  </span>
                  <span className="font-mono text-[9.5px] tracking-[0.08em] text-[var(--admin-muted)]">
                    · {a.branch}
                  </span>
                  <span className="ml-auto font-mono text-[9.5px] text-[var(--admin-muted)]">{a.time} ago</span>
                </div>
                <div className="text-[12.5px] leading-tight text-espresso">{a.text}</div>
              </div>
            </div>
          ))}
        </div>
