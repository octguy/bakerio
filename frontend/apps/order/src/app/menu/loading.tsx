...
export default function Loading() {
  return (
    <main className="relative isolate min-h-screen overflow-x-clip px-4 pt-3 pb-28 sm:px-6 md:px-8 md:pt-8 md:pb-16 xl:px-10">
      ...
      <div className="mx-auto grid w-full max-w-[1520px] animate-pulse gap-4 lg:items-start min-[1440px]:grid-cols-[minmax(360px,0.76fr)_minmax(0,1fr)] xl:gap-5 2xl:grid-cols-[minmax(420px,0.8fr)_minmax(0,1.2fr)]">
        <section className="min-w-0 min-[1440px]:sticky min-[1440px]:top-8">
          <MenuLocationHeader />
          <div className="mb-4 min-h-[270px] rounded-[2rem] bg-crust sm:min-h-[320px] lg:min-h-[520px] lg:rounded-[2.75rem] min-[1440px]:min-h-[calc(100vh-12rem)]" />
        </section>
        <section className="min-w-0 space-y-6 lg:pt-2">
          <div className="rounded-[1.75rem] border border-crust bg-cream/70 p-4 md:rounded-[2.25rem] md:p-5">
            <div className="mb-4 h-14 rounded-2xl bg-crust" />
            <div className="h-12 rounded-full bg-crust" />
            <div className="mt-4 flex gap-2 overflow-hidden md:flex-wrap">
              {Array.from({ length: 7 }).map((_, i) => (
                <div key={i} className="h-11 w-24 shrink-0 rounded-full bg-crust" />
              ))}
            </div>
          </div>
          <div className="grid grid-cols-1 gap-3 min-[380px]:grid-cols-2 sm:gap-4 md:grid-cols-3 lg:grid-cols-[repeat(auto-fit,minmax(min(100%,13.5rem),1fr))] 2xl:grid-cols-[repeat(auto-fit,minmax(min(100%,14.5rem),1fr))]">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="h-64 rounded-[1.65rem] bg-crust md:rounded-[2rem]" />
            ))}
          </div>
        </section>
      </div>
