...
async function MenuCatalog() {
  ...
  return loadError ? (
    <div role="alert" className="mt-6 rounded-2xl border border-sienna/30 bg-sienna/10 px-4 py-5 text-center">
      <h1 className="font-display text-[24px] leading-none tracking-tight text-espresso">{t("unavailable")}</h1>
...
export default async function MenuPage() {
  ...
  return (
    <main className="relative isolate min-h-screen overflow-x-clip bg-vanilla px-4 pt-3 pb-28 sm:px-6 md:px-8 md:pt-8 md:pb-16 xl:px-10">
      ...
      <MenuLayoutClient
        catalogSection={
          <Suspense fallback={<div className="py-12 text-center font-editorial text-[14.5px] italic text-caramel">{t("loadingBatch")}</div>}>
            <MenuCatalog />
          </Suspense>
        }
      />
