...
export default function LoginPage() {
  ...
  return (
    <main className="mx-auto max-w-md px-7 pt-10 pb-24">
      <span className="block font-slab italic text-[28px] leading-none text-cinnamon">{t("welcomeBack")}</span>
      <h1
      ...
      >
        {t("pickUpYour")}
        <br />
        <span className="font-editorial text-cinnamon">{t("order")}</span>
      </h1>
      ...
      <div className="mt-4 flex flex-col gap-2">
        {[{ l: t("continueApple") }, { l: t("continueGoogle") }].map((b) => (
          <button
            key={b.l}
            type="button"
            disabled
          ...
          >
            <span>{b.l}</span>
            <span className="rounded bg-caramel/10 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-[0.1em] text-caramel">{t("comingSoon")}</span>
          </button>
        ))}
      </div>
