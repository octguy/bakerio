...
const getUserBranchID = `-- name: GetUserBranchID :one
SELECT branch_id FROM auth.users WHERE id = $1
`

func (q *Queries) GetUserBranchID(ctx context.Context, id uuid.UUID) (*uuid.UUID, error) {
	row := q.db.QueryRow(ctx, getUserBranchID, id)
